...
\begin{figure}[H]
  ...
  \caption{CI Pipeline (GitHub Actions) with three consecutive successful runs, all with English commit messages (criterion P1), 2026-09-19. Commits \texttt{fa963ce}, \texttt{d81a0ab}, \texttt{2a8bd32} on \texttt{main}.}
...
\begin{figure}[H]
  ...
  \caption{Deployment on Render with status \texttt{Live} for commit \texttt{4722c1f} (3m47s), 2026-09-18. The greyed-out entry above it, for the same commit, was manually cancelled by the developer after it hung (10m34s with no progress); it is not a failed build, and the retry immediately below succeeded.}
