...
capítulo de implementación); no miden caché real. La media de avg por
corrida caliente es \textbf{3967.32\,ms} (DT 1539.20, IC\,95\,\%
[2056.46; 5878.19]) y la media fría es \textbf{257.50\,ms}. El p95 caliente
va de 4.3\,s a 11.4\,s; el plan gratuito de Render (0.1\,vCPU) no cumple el
...
\cite{jiang2015}. El contraste Mann-Whitney frío vs.\ caliente arroja
...
efecto de la caché de aplicación. La serie local K1 (p95 medio 81\,ms) sigue
siendo la referencia de rendimiento sin throttling.
...
una con una clave de caché distinta (\texttt{page size} 11 a 15) para
garantizar un \emph{miss} real e independiente en cada corrida
(\texttt{docs/mediciones/perf/k10}--\texttt{k14-cache-contrast.json}).
Recalculado desde los JSON crudos con
...
\midrule
Frío (avg, ms) & 1729.65 & 408.35 & 491.88 & 225.18 & 233.83 \\
Caliente (avg, ms) & 388.69 & 309.35 & 209.31 & 209.31 & 200.94 \\
\bottomrule
\end{tabular}
\end{center}

$U = 4{,}0$, $z = -1{,}78$, $p = 0{,}0758$ (bilateral), $d$ de Cliff $=
0{,}68$ (efecto grande). Con $n=5$ por condición el contraste no alcanza
significancia al 5\,\% ($p > 0{,}05$), pero el tamaño del efecto es
grande y consistente en dirección con K9: en las cinco corridas la
media fría fue mayor que la caliente, sin excepción. La falta de
significancia formal es un límite honesto del tamaño muestral pequeño
(el mismo patrón que \texttt{ANALISIS-k6.md} ya declara para el
contraste K1 local), no evidencia de que la caché no funcione.
