...
\begin{itemize}
  ...
  \item Las conclusiones de rendimiento en el entorno local (K1--K3) no miden
        un efecto de caché -- esa serie es anterior a la implementación real
        de \texttt{@Cacheable} (\S\ref{sec:perf}). El contraste caché
        caliente/fría con metodología válida (K9, K10--K14) sí existe, pero
        con $n=5$ no alcanza significancia estadística: no se puede concluir
        que el caché reduzca la latencia de forma consistente en este
        entorno, solo que el efecto, si existe, es menor que el ruido de red
